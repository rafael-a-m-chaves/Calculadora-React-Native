...
export default class App extends Component {
  ...
  addDigit = n => {
    if( n === '.' && this.state.values[this.state.current].toString().includes('.') ){
      return
    } 
  ...
  trocarSinal = () => {
    if( this.state.displayValue !== '0'){
      const values = [...this.state.values]
      const currenteValue = this.state.displayValue
      const newValue = (-1) * parseFloat(currenteValue)
    ...
    return (
      <View style ={styles.container}>
        ...
        <View style ={styles.buttons}>
          ...
          <Button label='+/-' onClick={this.trocarSinal}/>
